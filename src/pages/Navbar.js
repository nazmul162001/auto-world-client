...
const Navbar = ({ setShow, handleOpen }) => {
  ...
  const handleClickOutside = (e) => {
    if (!closeOutside.current.contains(e.target)) {
      setOpenSearch(false)
    } else {
      setOpenSearch(true)
    }
    ...
    <nav className='h-[70px] lg:h-[56px] border-b-2 bg-[#F5F5F5] border-[#E6E6E6] w-full sticky top-0 left-0'>
      <section className='w-full bg-green h-full justify-between grid grid-cols-3'>
        <div className='flex items-center justify-start col-span-1 md:col-span-2'>
          ...
          {/* mobile search bar  */}
          <span ref={closeOutside} className='lg:hidden ml-2'>
            <BsSearch className='text-xl text-softBlack cursor-pointer' />{' '}
          </span>
          {/* mobile search field */}
          {openSearch && (
            <div className='absolute h-[70px] w-full bg-white flex justify-center items-center transition-all duration-1000'>
